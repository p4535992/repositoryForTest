...
var FileLoader = L.Class.extend({
    ...
    options: {
        ...
        isgtfs: false               //if is a gtfs file..
    },
    ...
    load: function (file /* File */) {
        ...
        if (typeof file === 'undefined') { /*do nothing avoid console error*/
            ...
            if (ext == "zip") {  //if is a archive
                ...
                var parser;
                if(this.options.isgtfs) parser = this._parsers['gtfs'];
                else parser = this._parsers[ext];
                ...
                reader.onload = L.Util.bind(function (e) {
                    try {
                        ...
                        if(!this.options.isgtfs)this.fire('data:loaded', {layer: layer, filename: file.name, format: ext});
                    }
                    catch (err) {
                        //this.fire('data:error', {error: err});
    ...
    _gtfsToGEOJSON3: function (content) {
        if (typeof content == 'string') {
            content = JSON.parse(content);
        }
        ...
        scope.$apply(function(){
            scope.contentjAgency = content;
            scope.refresh(content);
        })
    },
    /* ************************************

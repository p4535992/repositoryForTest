...
System.register('lib/filelayer/leaflet.filelayer.js', ['npm:babel-runtime@5.8.34/core-js/object/keys', 'npm:leaflet@0.7.7'], function (_export) {
    var _Object$keys, L, FileLoader;

    return {
        setters: [function (_npmBabelRuntime5834CoreJsObjectKeys) {
            _Object$keys = _npmBabelRuntime5834CoreJsObjectKeys['default'];
        }, function (_npmLeaflet077) {
            L = _npmLeaflet077['default'];
        }],
        execute: function () {
            /*
            ...
            FileLoader = L.Class.extend({
                ...
                _csvToGeoJSON: function _csvToGeoJSON(content) {
                    try {
                        ...
                        if (this.options.titlesToInspect.length == 0) this._titles = json.meta.fields;else this._titles = this.options.titlesToInspect;
                        delete json.errors;
                        delete json.meta;
                ...
                _keyCount: function _keyCount(o) {
                    return typeof o == 'object' ? _Object$keys(o).length : 0;
                },

                _addFeatureToJson: function _addFeatureToJson(json) {
                    //be sure we have a json array of object of objects
                    if (json === null || typeof json === 'undefined' || _Object$keys(json).length == 0) {
                    ...
                    json = {
                        type: "FeatureCollection",
                        features: _Object$keys(json).map(function (id) {
                            //id 0,1,2,3,4,5,.....
                            var obj = json[id];
                            if (obj === null || typeof obj === 'undefined' || id >= _Object$keys(json).length - 1) {
                                console.warn("Ignore line ", id, " invalid data");
                                return;
                            } else {
                                //if you not have setted a specific set of columns just get everything
                                if (!titles.length > 0) titles = _Object$keys(obj);
                                                ...
                                                try {
                                                    if (obj.hasOwnProperty(title)) {
                                                        if (popupTable) {
                                                            ...
                                                            if (href.length > 0) content += '<tr><th>' + title + '</th><td>' + href + '</td></tr>';else content += '<tr><th>' + title + '</th><td>' + obj[title] + '</td></tr>';
                                                        } else {
                    ...
                    if (this.options.validateGeoJson) {
                        ...
                        if (ajax.result.isCorrect) return json;else console.error("The geo json generated is wrong:" + JSON.stringify(ajax.result.response, undefined, 2));
                    } else return json;
                },

                _RDFToGeoJSON: function _RDFToGeoJSON(content) {
                    try {
                        var xml = this._toXML(content);
                        var json = this._XMLToJSON(xml);

                        for (var i = 0; i < _Object$keys(this.options.rootTag).length; i++) {
                            json = json[this.options.rootTag[_Object$keys(this.options.rootTag)[i]]];
                        }
                ...
                _toXML: function _toXML(content) {
                    var xml;
                    try {
                        if (window.DOMParser) {
                ...
                _XMLToJSON: function _XMLToJSON(content) {
                    var attr,
                        child,
                ...
                _XMLToGeoJSON: function _XMLToGeoJSON(content) {
                    var xml = this._toXML(content);
                    var json = this._XMLToJSON(xml);
                ...
                _simplifyJson: function _simplifyJson(json) {
                    if (!(Object.prototype.toString.call(json) === '[object Array]')) {
                        this.fire('data:error', {
                            error: new Error('Try to simplify a not json array, please re-set your root tag path, ' + 'e.g. xmlRootTag:["some","pathTo","Array"], we need a json array')
                        });
                        return;
                    }
                    var root = { data: [] };
                    for (var i = 0; i < _Object$keys(json).length; i++) {
                        //406 object
                        var obj;
                        ...
                        try {
                            var elements;
                            if (_Object$keys(obj).length > 1) elements = _Object$keys(obj).toString().split(",");else elements = _Object$keys(obj).toString();
                            for (var element, k = 0; element = elements[k++];) {
                                ...
                                if (element.toString() == "#text") {
                                    ...
                                    key = _Object$keys(obj[element]); //rdf:
                                    value = obj[element][key].toString();
                                } else {
                                    key = element;
                                    value = _Object$keys(obj[element]).toString();
                                    if (value == "@attributes") {
                                        value = obj[element]["@attributes"][_Object$keys(obj[element]["@attributes"])];
                                    } else if (value == "#text") {
                                        value = obj[element]["#text"];
                                    } else if (value == "@attributes,#text") {
                                        value = obj[element]["#text"];
                                        info[key] = value;
                                        key = _Object$keys(obj[element]["@attributes"]);
                                        value = obj[element]["@attributes"][_Object$keys(obj[element]["@attributes"])];
                                        //info[key] = value;
                                    } else {
                                            //never run here.....
                ...
                _mergeRdfJson: function _mergeRdfJson(json) {
                    try {
                        var link = '';
                        var mJson = [];
                        var xJson;
                        ...
                        for (var i = 0; i < _Object$keys(json).length; i++) {
                            for (var k = 0; k < _Object$keys(this.options.rdfLink).length; k++) {
                                if (json[i].hasOwnProperty(this.options.rdfLink[_Object$keys(this.options.rdfLink)[k]])) {
                        ...
                        for (i = 0; i < _Object$keys(json).length; i++) {
                ...
                _searchJsonByKeyAndValue: function _searchJsonByKeyAndValue(json, key, value) {
                ...
                _mergeJson: function _mergeJson(json1, json2) {
                    for (var key in json2) if (json2.hasOwnProperty(key)) json1[key] = json2[key];
                    return json1;
                },

                _removeNullJson: function _removeNullJson(json) {
                    // Compact arrays with null entries; delete keys from objects with null value
                    var isArray = json instanceof Array;
                    for (var k in json) {
                        //type,properties,..,title,popupContent,features,..
                        if (json[k] == null || typeof json[k] === 'undefined') isArray ? json.splice(k, 1) : delete json[k];else if (typeof json[k] == "object") this._removeNullJson(json[k]);
                    }
                },

                _cleanJson: function _cleanJson(json) {
                ...
                _gtfsToGeoJSON: function _gtfsToGeoJSON(content) {
                    ...
                    for (item, i = 0; item = dintintShape[i++];) {
                        if (item.length > 0 && item != '') {
                            //avoid null object
                            json[item] = [];
                            for (var k = 0; k < _Object$keys(shapes).length; k++) {
                            ...
                            return {
                                ...
                                geometry: {
                                    ...
                                    geometries: [{
                                        ...
                                        coordinates: (function () {
                                            var coords = [];
                                            for (var s = 0; s < _Object$keys(json[id]).length; s++) {
                ...
                _gtfsZipToGEOJSON: function _gtfsZipToGEOJSON(file) {
                    parseGtfs(file, {
            ...
            L.Control.FileLayerLoad = L.Control.extend({
                ...
                _initDragAndDrop: function _initDragAndDrop(map) {
                    ...
                    for (var name in callbacks) dropbox.addEventListener(name, callbacks[name], false);
                },
            ...
            if (typeof module === 'object' && typeof module.exports === 'object') {
                module.exports = {
                    //'control.fileLayerLoad': L.Control.fileLayerLoad,
                    FileLayerLoad: L.Control.FileLayerLoad,
            ...
            _export('default', function (options) {
                return L.Control.FileLayerLoad(options);
            });
...
System.register('lib/main.js', ['npm:leaflet@0.7.7/dist/leaflet-src', 'npm:leaflet@0.7.7', 'lib/filelayer/leaflet.filelayer.js'], function (_export) {
    ...
    return {
        ...
        execute: function () {
            ...
            map = new L.Map("map", { center: [35.78, -78.68], zoom: 13 });

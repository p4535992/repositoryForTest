...
(function(process) {
  ...
  function kmlFixtureEqual(t, file) {
    ...
    t.equal(fs.readFileSync(file + '.geojson', 'utf8'), JSON.stringify(tj.kml(toDOM(fs.readFileSync(file))), null, 4), file);
  }
  function gpxFixtureEqual(t, file) {
    ...
    t.deepEqual(fs.readFileSync(file + '.geojson'), tj.gpx(toDOM(fs.readFileSync(file))), file);
  }
  test('KML', function(t) {

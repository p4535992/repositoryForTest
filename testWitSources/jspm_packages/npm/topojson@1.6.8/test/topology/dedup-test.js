...
suite.addBatch({"dedup": {
    ...
    "ring ABA has no cuts": function() {
      var topology = dedup(cut(extract({aba: {
          type: "Polygon",
          coordinates: [[[0, 0], [1, 0], [0, 0]]]
        }})));
      assert.deepEqual(topology.objects, {aba: {
          type: "Polygon",
          arcs: [{
            0: 0,
            1: 2
    ...
    "ring AA has no cuts": function() {
      var topology = dedup(cut(extract({aa: {
          type: "Polygon",
          coordinates: [[[0, 0], [0, 0]]]
        }})));
      assert.deepEqual(topology.objects, {aa: {
          type: "Polygon",
          arcs: [{
            0: 0,
            1: 1
    ...
    "degenerate ring A has no cuts": function() {
      var topology = dedup(cut(extract({a: {
          type: "Polygon",
          coordinates: [[[0, 0]]]
        }})));
      assert.deepEqual(topology.objects, {a: {
          type: "Polygon",
          arcs: [{
            0: 0,
            1: 0
    ...
    "when a line ABCDBE self-intersects with its middle, it is not cut": function() {
      var topology = dedup(cut(extract({abcdbe: {
          type: "LineString",
          coordinates: [[0, 0], [1, 0], [2, 0], [3, 0], [1, 0], [4, 0]]
        }})));
      assert.deepEqual(topology.objects, {abcdbe: {
          type: "LineString",
          arcs: {
            0: 0,
            1: 5
    ...
    "when a line ABACD self-intersects with its start, it is cut into ABA-ACD": function() {
      var topology = dedup(cut(extract({abacd: {
          type: "LineString",
          coordinates: [[0, 0], [1, 0], [0, 0], [3, 0], [4, 0]]
        }})));
      assert.deepEqual(topology.objects, {abacd: {
          type: "LineString",
          arcs: {
            0: 0,
            1: 2,
    ...
    "when a line ABDCD self-intersects with its end, it is cut into ABD-DCD": function() {
      var topology = dedup(cut(extract({abdcd: {
          type: "LineString",
          coordinates: [[0, 0], [1, 0], [4, 0], [3, 0], [4, 0]]
        }})));
      assert.deepEqual(topology.objects, {abdcd: {
          type: "LineString",
          arcs: {
            0: 0,
            1: 2,
    ...
    "when a line ABCA is closed, there are no cuts": function() {
      var topology = dedup(cut(extract({abca: {
          type: "LineString",

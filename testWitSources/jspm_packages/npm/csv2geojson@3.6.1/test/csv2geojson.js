...
describe('csv2geojson', function() {
  ...
  describe('#csv2geojson', function() {
    ...
    describe('delimiters', function() {
      it('|', function(done) {
        csv2geojson.csv2geojson(textFile('simple.pipe.dsv'), {delimiter: '|'}, function(err, data) {
          expect(data).to.eql(jsonFile('simple.geojson'));
          done();
        });
      });
      it(',', function(done) {
        csv2geojson.csv2geojson(textFile('simple.csv'), {delimiter: ','}, function(err, data) {
          expect(data).to.eql(jsonFile('simple.geojson'));
          done();
        });
      });
      it(';', function(done) {
        csv2geojson.csv2geojson(textFile('simple.semicolon.dsv'), {delimiter: ';'}, function(err, data) {
          expect(data).to.eql(jsonFile('simple.geojson'));
          done();
        });
      });
      it('tab', function(done) {
        csv2geojson.csv2geojson(textFile('simple.tsv'), {delimiter: '\t'}, function(err, data) {
          expect(data).to.eql(jsonFile('simple.geojson'));
          done();
        });
      });
      describe('auto', function() {
        ['simple.tsv', 'simple.semicolon.dsv', 'simple.csv', 'simple.pipe.dsv'].forEach(function(f) {
          it('auto with ' + f, function(done) {
            csv2geojson.csv2geojson(textFile(f), {delimiter: 'auto'}, function(err, data) {
              expect(data).to.eql(jsonFile('simple.geojson'));
              done();
            });
          });
        ...
        it('no match', function(done) {
          csv2geojson.csv2geojson('', {delimiter: 'auto'}, function(err, data) {
            expect(err).to.eql({
              type: 'Error',

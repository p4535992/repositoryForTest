...
test('write works as expected', function(assert) {
  var tests = {
    ...
    writeDoubleLE: [binary.from([0xcf, 0xaf, 0xda, 0x02, 0x00, 0x01, 0x2f, 0x44]), 285960563508654870000],
  ...
  for (var key in tests) {
    ...
    for (var i = 0,
        len = expect.length; i < len; ++i) {
      assert.equal(binary.readUInt8(buf, i), binary.readUInt8(expect, i));
    }
  }
  assert.end();
});

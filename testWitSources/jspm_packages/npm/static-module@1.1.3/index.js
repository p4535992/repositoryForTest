...
module.exports = function parse(modules, opts) {
  ...
  function walk(node) {
    ...
    } else if (isreqv && node.parent.type === 'MemberExpression' && node.parent.property.type === 'Identifier' && node.parent.parent.type === 'VariableDeclarator' && node.parent.parent.id.type === 'Identifier') {
      var v = varModules[reqid][node.parent.property.name];
      vars[node.parent.parent.id.name] = v;
    } else if (isreqv && node.parent.type === 'MemberExpression' && node.parent.property.type === 'Identifier') {} else if (isreqv && node.parent.type === 'CallExpression') {}
    if (isreqm && node.parent.type === 'VariableDeclarator' && node.parent.id.type === 'Identifier') {
      ...
      if (decs.length) {
        ...
        updates.push({
          ...
          offset: node.parent.parent.end + 1 - node.parent.parent.start,
          stream: st('var ')
        });
        decs.forEach(function(d, i) {
          ...
          if (i < decs.length - 1) {
            ...
            if (i === ix - 1) {
              ...
              comma = body.slice(d.end, decs[i + 1].start);
            updates.push({
              start: d.end,
      ...
      if (cur.type === 'SequenceExpression') {
        ...
        if (ix >= 0)
          ex.splice(ix, 1);
        pushUpdate(node.parent.parent, unparse(node.parent.parent));
      } else
        pushUpdate(cur, '');
    } else if (isreqm && node.parent.type === 'MemberExpression' && node.parent.property.type === 'Identifier' && node.parent.parent.type === 'VariableDeclarator' && node.parent.parent.id.type === 'Identifier') {
      varNames[node.parent.parent.id.name] = [reqid, node.parent.property.name];
      ...
      if (decs.length === 0) {
        ...
        pushUpdate(node.parent.parent.parent, unparse(node.parent.parent.parent));
      }
    } else if (isreqm && node.parent.type === 'MemberExpression' && node.parent.property.type === 'Identifier') {

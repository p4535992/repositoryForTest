  ...
  addData: function (data) {
    if (typeof data === 'string') {
      ...
      if (this.options.firstLineTitles) {
        ...
        title = title.trim().split(this.options.fieldSeparator);
        for (var i=0; i<title.length; i++) {
          title[i] = this._deleteDoubleQuotes(title[i]);
        }
        this.options.titles = title;
      }
      //generamos _propertiesNames
      for (var i=0; i<title.length; i++) {
         var prop = title[i].toLowerCase().replace(/[^\w ]+/g,'').replace(/ +/g,'_');

...
RepositoryConnection con = repo.getConnection();
	...
	String queryString = "PREFIX SiiMobility:<http://www.disit.dinfo.unifi.it/SiiMobility#> " + 
			...
			"	SELECT distinct ?via ?numero ?comune" + 
			"	WHERE { " + 
			...
			"	?road SiiMobility:extendName ?via . " + 
			"	?entry geo:lat ?elat . " + 
			"	 ?entry geo:long ?elong . " + 
			"	?road SiiMobility:inMunicipalityOf ?com . " +
			"	?com foaf:name ?comune . " +
			"	?entry omgeo:nearby(" + latitudine + " " + longitudine + "  \"0.1km\") . " + 
			" BIND( omgeo:distance(?elat, ?elong, " + latitudine + ", " + longitudine + ") AS ?distanza)	" + 
			"	} " +  
			"ORDER BY ?distanza " + 
			"LIMIT 1";
	...
	try{
		...
		if (result.hasNext()) {
			BindingSet bindingSet = result.next();
			 String valueOfVia = bindingSet.getValue("via").toString();
			 String valueOfNumero = bindingSet.getValue("numero").toString();
			 String valueOfComune = bindingSet.getValue("comune").toString();
			 
			 valueOfVia = valueOfVia.replace("\"^^<http://www.w3.org/2001/XMLSchema#string>", "");
			 valueOfVia = valueOfVia.replace("\"", "");
			 
			 valueOfNumero = valueOfNumero.replace("\"^^<http://www.w3.org/2001/XMLSchema#string>", "");
			 valueOfNumero = valueOfNumero.replace("\"", "");
			 
			 valueOfComune = valueOfComune.replace("\"^^<http://www.w3.org/2001/XMLSchema#string>", "");
			 valueOfComune = valueOfComune.replace("\"", "");
